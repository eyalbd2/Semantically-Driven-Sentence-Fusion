...
def _eval_epoch(sess, epoch, mode):
    ...
    if mode == 'eval':
        ...
        logger.info('epoch: %d, exact score %.4f, type accuracy %.2f, conn accuracy %.2f', epoch + 1, eval_exact,
                    eval_types_acc, eval_conns_acc)
        print('epoch: %d, exact score %.4f, type accuracy %.2f, conn accuracy %.2f' % (epoch + 1, eval_exact,
                                                                                       eval_types_acc, eval_conns_acc))
